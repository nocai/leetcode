/*
 * @lc app=leetcode.cn id=142 lang=golang
 *
 * [142] 环形链表 II
 *
 * https://leetcode-cn.com/problems/linked-list-cycle-ii/description/
 *
 * algorithms
 * Medium (51.71%)
 * Total Accepted:    114.4K
 * Total Submissions: 221.2K
 * Testcase Example:  '[3,2,0,-4]\n1'
 *
 * 给定一个链表，返回链表开始入环的第一个节点。 如果链表无环，则返回 null。
 *
 * 为了表示给定链表中的环，我们使用整数 pos 来表示链表尾连接到链表中的位置（索引从 0 开始）。 如果 pos 是 -1，则在该链表中没有环。
 *
 * 说明：不允许修改给定的链表。
 *
 *
 *
 * 示例 1：
 *
 * 输入：head = [3,2,0,-4], pos = 1
 * 输出：tail connects to node index 1
 * 解释：链表中有一个环，其尾部连接到第二个节点。
 *
 *
 *
 *
 * 示例 2：
 *
 * 输入：head = [1,2], pos = 0
 * 输出：tail connects to node index 0
 * 解释：链表中有一个环，其尾部连接到第一个节点。
 *
 *
 *
 *
 * 示例 3：
 *
 * 输入：head = [1], pos = -1
 * 输出：no cycle
 * 解释：链表中没有环。
 *
 *
 *
 *
 *
 *
 * 进阶：
 * 你是否可以不用额外空间解决此题？
 *
 */
/**
 * Definition for singly-linked list.
 * type ListNode struct {
 *     Val int
 *     Next *ListNode
 * }
 */
func detectCycle(head *ListNode) *ListNode {
	// return detectCycle_hash(head)
	return detectCycle_floyd(head)
}

func detectCycle_hash(head *ListNode) *ListNode {
	mp := map[*ListNode]struct{}{}

	for head != nil {
		if _, exist := mp[head]; exist {
			return head
		}
		mp[head] = struct{}{}
		head = head.Next
	}
	return nil
}

func detectCycle_floyd(head *ListNode) *ListNode {
	if head == nil {
		return nil
	}

	intersect := findIntersect(head)
	if intersect == nil {
		return nil
	}

	begin, end := head, intersect
	for begin != end {
		begin = begin.Next
		end = end.Next
	}
	return begin
}

// 寻找相遇点
func findIntersect(head *ListNode) *ListNode {
	slow, fast := head, head

	for fast != nil && fast.Next != nil {
		slow = slow.Next      // 慢的跑一步
		fast = fast.Next.Next // 快的跑两步
		if fast == slow {
			return fast
		}
	}

	return nil
}
